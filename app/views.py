...
def course_page(request, slug):
    ...
    context = dict(
            context.items() + course_page_context(request, course).items())
...
def vote_course(request):
    ...
    if rating_type != PROFESSOR_R:
        ...
        ratings = Professor_Rating.objects.filter(user=user, course=course,
                                                  rating_type=rating_type,
                                                  prof=prof)
        if len(ratings) == 0:
            rating = Professor_Rating(user=user, course=course,
                                      rating=rating_value,
                                      rating_type=rating_type, prof=prof)
            rating.save()
        else:
            rating = ratings[0]
            rating.rating = rating_value
...
def compare_next(request, slug1=""):
    ...
    message = ""
    if not slug1:
        message = "Select a course that you want to compare by clicking it's " \
                  "name"
    else:
        course1 = get_object_or_404(Course, slug=slug1)
        message = "You selected course '" + course1.name + "'(<a " \
                                                           "href='/compare_course/'>go back</a>). Select the course that you want to compare it with"
...
def login_action(request):
    ...
    if len(users) == 0:
        user = jUser.objects.create_user(username=login_username,
                                         password="1234")
        user.save()
    ...
    if user is not None:
        if user.is_active:
            ...
            context[
                'error'] = "Invalid user! Please try again! The account may " \
                           "not be activated!"
            return render(request, "pages/login_page.html", context)
    else:
